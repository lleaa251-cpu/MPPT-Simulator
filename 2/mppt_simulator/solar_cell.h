...
struct SolarCell {
    ...
    double thermalVoltage(double T_K) const;  // 計算熱電壓 Vt = n*k*T/q
    double photocurrent(double G, double T_K) const;  // 計算光生電流 Iph
    double calcCurrent(double V, double G, double T_C) const;  // 牛頓法迭代：給定 V、G(W/m²)、T(°C)，求 I(A)
    double calcVoc(double G, double T_C) const;  // 計算開路電壓 Voc（二分法）
};
